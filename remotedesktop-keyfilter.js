(function () {
    ...
    Memory.scan(Module.findBaseAddress("mstscax.dll"), 500000, "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 8B F1", {
      onMatch: (addr, size) => { keyboardProc = addr; return "stop"; },
      onComplete: () => { InstallHook(keyboardProc); }
    });
  
    function InstallHook(keyboardProcAddress) {
      var originalKeyboardProc = new NativeFunction(keyboardProcAddress, 'int64', ['int', 'uint64', 'pointer'], 'win64');
      ...
      Interceptor.replace(keyboardProcAddress, new NativeCallback(function (nCode, wParam, lParam) {
